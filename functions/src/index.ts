...
async function sendAdminNotification(bookingData: BookingData, bookingId: string): Promise<void> {
  const mailOptions = {
...
async function sendCustomerConfirmation(bookingData: BookingData, bookingId: string): Promise<void> {
  const mailOptions = {
    from: `${COMPANY_NAME} <${ADMIN_EMAIL}>`,
        ...
        <div style="padding: 30px; background-color: #ffffff; border: 1px solid #e5e7eb; border-top: none;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            ...
            <p style="color: #374151;">
              ...
              📞 Phone: ${functions.config().company?.phone || 'Contact us via email'}
            </p>
          </div>
        </div>
...
async function sendStatusUpdateNotification(bookingData: BookingData, bookingId: string, oldStatus: string): Promise<void> {
